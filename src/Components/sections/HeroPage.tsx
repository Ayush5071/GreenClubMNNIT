...
function HeroPage() {
  ...
  return (
    <WavyBackground
      ...
      containerClassName="h-screen flex flex-col items-center pt-20 pb-16 justify-center"
      colors={["#22c55e","#16a34a","#15803d","#65a30d","#84cc16","#a3e635","#059669","#047857"]}
    ...
    >
      ...
      >
        ...
        <div className="space-y-2 mobile:space-y-1">
          <h1 className="text-3xl font-bebas sm:text-5xl md:text-7xl lg:text-8xl text-white font-extrabold tracking-wide drop-shadow-2xl bg-gradient-to-r from-white via-green-100 to-green-200 bg-clip-text text-transparent mobile:text-xl">
            GREEN CLUB
          </h1>
          <p className="text-base font-bebas sm:text-2xl md:text-3xl lg:text-4xl text-green-300 font-bold tracking-wider drop-shadow-md mobile:text-xs">
            MNNIT ALLAHABAD
          </p>
        </div>
        
        {/* Mission Statement */}
        <div className="max-w-4xl mx-auto space-y-3 mobile:space-y-1">
          <p className="text-xl md:text-3xl text-white font-Sfpro font-semibold leading-relaxed drop-shadow-md mobile:text-base">
          ...
          <p className="text-base md:text-xl text-gray-200 font-Sfpro leading-relaxed max-w-3xl mx-auto mobile:text-xs">
        ...
        <div className="flex flex-wrap justify-center gap-4 mt-12 hidden sm:flex">
          {activities.map((activity, index) => (
            <div
              key={index}
              className={`flex items-center space-x-3 bg-white/10 backdrop-blur-sm rounded-full px-6 py-3 border border-white/20 hover:bg-white/20 hover:border-green-300/50 transition-all duration-300 transform hover:scale-105 hover:shadow-lg cursor-pointer group mobile:px-3 mobile:py-1 mobile:text-xs ${
